                  ...
                  _SettingsEntry(
                    ...
                    child: TextButton(
                      ...
                      onPressed: () async {
                        ...
                        // Android 4.4 (KitKat): the SAF directory picker is API 21+,
                        // so use the in-app browser (returns the chosen folder path).
                        if (checkPlatform([TargetPlatform.android]) &&
                            (ref.read(deviceRawInfoProvider).androidSdkInt ??
                                    0) <
                                21) {
                          final picked = await context.push(() =>
                              const FileBrowserPage(
                                  selectFolder: true, returnPath: true));
                          if (picked is String) {
                            await ref
                                .notifier(settingsProvider)
...
/// A row of preset seed-color swatches. Lets the user pick a custom theme color
/// (needed on Android < 12 / KitKat, which can't read the system accent color).
class _ColorSeedPicker extends StatelessWidget {
  const _ColorSeedPicker();

  // "teal" is the LocalSend default and is stored as null.
