...
class MainActivity : FlutterActivity() {
    ...
    private fun openDirectoryPicker() {
        // ACTION_OPEN_DOCUMENT_TREE and the tree-based DocumentsContract APIs are API 21+.
        // Android 4.4 (KitKat) has no SAF tree picker, so fail gracefully instead of crashing.
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            pendingResult?.error("UNSUPPORTED_API", "Picking a folder requires Android 5.0+.", null)
            pendingResult = null
            return
        }
        ...
        try {
            ...
            pendingResult?.error("PICK_DIRECTORY_FAILED", e.message, null)
            pendingResult = null
        }
    }
